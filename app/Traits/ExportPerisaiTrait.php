...
trait ExportPerisaiTrait
{
    ...
    private function exportWord($request, $kategori_valid)
    {
        ...
        $daftarKategoriMap = [
            'Fasilitas' => 'Sarpras (Sarana dan Prasarana)',
            'Paspor' => 'Pelayanan Paspor',
            'IzinTinggal' => 'Pelayanan Izin Tinggal',
            'TPI' => 'TPI (Tempat Pemeriksaan Imigrasi)'
        ];

        $no_kat = 1;
        foreach ($kategori_valid as $kat) {
            $raw_vals = $request->input("val_{$kat}") ?? [];
            $skor = count($raw_vals) > 0 ? (array_sum($raw_vals) / (count($raw_vals) * 5)) * 100 : 0;
    ...
    public function exportWordRiwayat($id)
    {
        ...
        $details = DB::table('detail_penilaian')
            ->join('soal_perisai', 'detail_penilaian.soal_id', '=', 'soal_perisai.id')
            ->where('detail_penilaian.riwayat_id', $id)
            ->select('soal_perisai.kategori', 'detail_penilaian.skor')
            ->get()
            ->groupBy('kategori');
